...
class Ball:
    def __init__(self, platform, canvas, color):
        self.canvas = canvas
        self.platform = platform
        self.oval = canvas.create_oval(200, 200, 215, 215, fill=color)
        self.dir = [-5,-4,-3,-2,-1,1,2,3,4,5]
        self.x = random.choice(self.dir)
        self.y = -1
        self.touch_bottom = False

    def touch_plat(self, ball_pos):
...
class Platform:
    ...
    def draw(self):
        canvas.move(self.rect, self.x, 0)
...
platform=Platform(canvas, 'grey')
ball = Ball(platform, canvas, 'purple')
